...
/* Fill the internal buffer. If an error occurs -1 is returned. Only
read once, since we may have read all and an extra read would just
be an extra system call. The timeout parameter is number of millisec to
wait for data. Returns true (the length of data read), -1 if an error 
occured or if the connection was closed by the client. 0 is returned
if a read returned 0 (eof) and read should be retried because it would
block. If an error occured the stream is also set in closed mode. */
static inline int fill(T S, time_t timeout) {
        if (S->isClosed)
                return -1;
        errno = 0;
        int n = (int)Net_read(S->fd, S->buffer + S->length, BUFFER_SIZE - S->length, timeout);
        if (n > 0)
                S->length += n;
        else if (n < 0) {
                n = -1;
...
/* Read a single byte. The byte is returned as an int in the range 0 to 255.
 Returns the byte read, or -1 if the end of the stream has been reached */
