...
boolean_t File_isFile(const char *file) {
        if (file) {
                struct stat buf;
                return (lstat(file, &buf) == 0 && S_ISREG(buf.st_mode));
        }
        return false;
}


boolean_t File_isDirectory(const char *file) {
        if (file) {
                struct stat buf;
                return (lstat(file, &buf) == 0 && S_ISDIR(buf.st_mode));
        }
        return false;
}


boolean_t File_exist(const char *file) {
        if (file) {
                struct stat buf;
                return (lstat(file, &buf) == 0);
        }
        return false;
}
...
boolean_t File_isReadable(const char *file) {
        if (file) 
...
boolean_t File_isWritable(const char *file) {
        if (file) 
...
boolean_t File_isExecutable(const char *file) {
        if (file) 
...
boolean_t File_delete(const char *file) {
        if (file)
...
boolean_t File_rename(const char *file, const char *name) {
        if (file)                
